...
ansible-playbook -i inventory/hosts playbook/new-ubuntu-instance.yml $1


my_ssh_key_path=$(cat vars/new-ubuntu-instance_vars.yml |grep my_ssh_key_path | awk '{print $2}')

echo "
[nginx_php]" >> inventory/hosts
...
c=0;
###> Create the inventory/hosts file for ansible local with public Ip addresses
for h in $hosts;
do
	echo "$h  ansible_ssh_private_key_file=$my_ssh_key_path" >> inventory/hosts
...
wait 30 
ansible-playbook -u ubuntu -i inventory/hosts playbook/server_setup.yaml $1  ###> basic ubuntu setup 
ansible-playbook -u ubuntu -i inventory/hosts playbook/compose.yml $1  ###> compose and run nginx
